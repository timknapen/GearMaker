...
class ofxVec2f {
  ...
  public:
    ...
    ofxVec2f( const ofxVec2f& pnt );


    // Getters and Setters.
    //
    void set( float _x, float _y );
    void set( const ofxVec2f& vec );
    float &operator[]( const int& i );


    // Check similarity/equality.
    //
    bool operator==( const ofxVec2f& vec );
    bool operator!=( const ofxVec2f& vec );
    bool match( const ofxVec2f& vec, float tollerance=0.0001 );
    /**
    ...
    */
    ...
    //
    void 	  operator=( const ofxVec2f& vec );
    ofxVec2f  operator+( const ofxVec2f& vec ) const;
    ofxVec2f& operator+=( const ofxVec2f& vec );
    ofxVec2f  operator-( const ofxVec2f& vec ) const;
    ofxVec2f& operator-=( const ofxVec2f& vec );
    ofxVec2f  operator*( const ofxVec2f& vec ) const;
    ofxVec2f& operator*=( const ofxVec2f& vec );
    ...
    //
    ofxVec2f  getRotated( float angle, const ofxVec2f& pivot ) const;
    ofxVec2f& rotate( float angle, const ofxVec2f& pivot );
    ofxVec2f  getRotatedRad( float angle, const ofxVec2f& pivot ) const;
    ofxVec2f& rotateRad( float angle, const ofxVec2f& pivot );
        

    // Map point to coordinate system defined by origin, vx, and vy.
    //
    ofxVec2f getMapped( const ofxVec2f& origin,
                        const ofxVec2f& vx,
                        const ofxVec2f& vy ) const;
    ofxVec2f& map( const ofxVec2f& origin,
                   const ofxVec2f& vx, const ofxVec2f& vy );


    // Distance between two points.
    //
    float distance( const ofxVec2f& pnt) const;
    float squareDistance( const ofxVec2f& pnt ) const;
    ...
    */
    ofxVec2f   getInterpolated( const ofxVec2f& pnt, float p ) const;
    ofxVec2f&  interpolate( const ofxVec2f& pnt, float p );
    ofxVec2f   getMiddle( const ofxVec2f& pnt ) const;
    ofxVec2f&  middle( const ofxVec2f& pnt );
    ofxVec2f&  average( const ofxVec2f* points, int num );
    ...
    */
    ...
    // getInterpolated
    ofxVec2f interpolated( const ofxVec2f& pnt, float p ) const;
    
    // getMiddled
    ofxVec2f middled( const ofxVec2f& pnt ) const;
    
    // getMapped 
    ofxVec2f mapped( const ofxVec2f& origin, const ofxVec2f& vx, const ofxVec2f& vy ) const;
    
    // squareDistance
    float distanceSquared( const ofxVec2f& pnt ) const;
    
    // use getRotated
    ofxVec2f rotated( float angle, const ofxVec2f& pivot ) const;    
};
...
inline ofxVec2f::ofxVec2f( const ofxVec2f& pnt ) {
	x = pnt.x;
	y = pnt.y;
}
...
inline void ofxVec2f::set( const ofxVec2f& vec ) {
	x = vec.x;
	y = vec.y;
}
...
//
//
inline bool ofxVec2f::operator==( const ofxVec2f& vec ) {
...
inline bool ofxVec2f::match( const ofxVec2f& vec, float tollerance ) {
...
//
//
inline void ofxVec2f::operator=( const ofxVec2f& vec ){
	x = vec.x;
	y = vec.y;
}

inline ofxVec2f ofxVec2f::operator+( const ofxVec2f& vec ) const {
	return ofxVec2f( x+vec.x, y+vec.y);
}

inline ofxVec2f& ofxVec2f::operator+=( const ofxVec2f& vec ) {
	x += vec.x;
	y += vec.y;
	return *this;
}
...
inline ofxVec2f ofxVec2f::rotated( float angle, const ofxVec2f& pivot ) const {
	return getRotated(angle, pivot);
}

inline ofxVec2f ofxVec2f::getRotated( float angle, const ofxVec2f& pivot ) const {
	float a = (float)(angle * DEG_TO_RAD);
	return ofxVec2f( ((x-pivot.x)*cos(a) - (y-pivot.y)*sin(a)) + pivot.x,
                     ((x-pivot.x)*sin(a) + (y-pivot.y)*cos(a)) + pivot.y );
}

inline ofxVec2f& ofxVec2f::rotate( float angle, const ofxVec2f& pivot ) {
	float a = (float)(angle * DEG_TO_RAD);
...
inline ofxVec2f ofxVec2f::getRotatedRad( float angle, const ofxVec2f& pivot ) const {
	float a = angle;
	return ofxVec2f( ((x-pivot.x)*cos(a) - (y-pivot.y)*sin(a)) + pivot.x,
                     ((x-pivot.x)*sin(a) + (y-pivot.y)*cos(a)) + pivot.y );
}

inline ofxVec2f& ofxVec2f::rotateRad( float angle, const ofxVec2f& pivot ) {
	float a = angle;
	float xrot = ((x-pivot.x)*cos(a) - (y-pivot.y)*sin(a)) + pivot.x;
	y = ((x-pivot.x)*sin(a) + (y-pivot.y)*cos(a)) + pivot.y;
	x = xrot;
...
inline ofxVec2f ofxVec2f::mapped( const ofxVec2f& origin,
					              const ofxVec2f& vx,
					              const ofxVec2f& vy ) const{
	return getMapped(origin, vx, vy);
}

inline ofxVec2f ofxVec2f::getMapped( const ofxVec2f& origin,
				                     const ofxVec2f& vx,
				                     const ofxVec2f& vy ) const
{
...
inline ofxVec2f& ofxVec2f::map( const ofxVec2f& origin,
				                const ofxVec2f& vx, const ofxVec2f& vy )
{
...
//
//
inline float ofxVec2f::distance( const ofxVec2f& pnt) const {
	float vx = x-pnt.x;
	float vy = y-pnt.y;
...
inline float ofxVec2f::distanceSquared( const ofxVec2f& pnt ) const {
	return squareDistance(pnt);
}

inline float ofxVec2f::squareDistance( const ofxVec2f& pnt ) const {
	float vx = x-pnt.x;
	float vy = y-pnt.y;
	return vx*vx + vy*vy;
}
...
inline ofxVec2f ofxVec2f::interpolated( const ofxVec2f& pnt, float p ) const{
	return getInterpolated(pnt, p);
}

inline ofxVec2f ofxVec2f::getInterpolated( const ofxVec2f& pnt, float p ) const {
	return ofxVec2f( x*(1-p) + pnt.x*p, y*(1-p) + pnt.y*p );
}

inline ofxVec2f& ofxVec2f::interpolate( const ofxVec2f& pnt, float p ) {
	x = x*(1-p) + pnt.x*p;
	y = y*(1-p) + pnt.y*p;
	return *this;
}
...
inline ofxVec2f ofxVec2f::middled( const ofxVec2f& pnt ) const{
	return getMiddle(pnt);
}

inline ofxVec2f ofxVec2f::getMiddle( const ofxVec2f& pnt ) const {
	return ofxVec2f( (x+pnt.x)/2.0f, (y+pnt.y)/2.0f );
}

inline ofxVec2f& ofxVec2f::middle( const ofxVec2f& pnt ) {
	x = (x+pnt.x)/2.0f;
	y = (y+pnt.y)/2.0f;
	return *this;
}
...
//
//
inline ofxVec2f& ofxVec2f::average( const ofxVec2f* points, int num ) {
	x = 0.f;
	y = 0.f;
	for( int i=0; i<num; i++) {
		x += points[i].x;
